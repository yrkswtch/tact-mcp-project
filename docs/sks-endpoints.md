...
**Step 3: 教室選択（複数教室の場合）**
- POST `/cgi-bin/s2login.pl` data: `cmd=jsx&param={"cmd":"login","kcd":"{教室コード}","loginid":"...","loginpw":"<encrypted>","tantoshacd":"...","usersm":"...","accesslv":"..."}`
...
| 請求取消処理 | `IEB300.wpp` |
| 滞留債権処理 | `IEB310.wpp` |
| コンビニ未入金一覧 | `IEV140.wpp` | ← MCPツール: `sks_cvs_unpaid_list`
| コンビニ入金済一覧 | `IEV250.wpp` | ← MCPツール: `sks_cvs_paid_list`
...
| 生徒氏名 | ✓ | テキスト | |
| 保護者氏名 | ✓ | テキスト | フルネーム必須。トップレベル（IEB010では家族構成テーブル内） |
| **フリガナ(半角カナ) (`seitokm`)** | **✓** | テキスト | 全角→半角自動変換。**空だとサーバー側で弾かれる必須フィールド** |
| フリガナ(英字) | | テキスト | |
| 性別 | | プルダウン | 男/女（既定：男） |
...
### 内部生情報の取得・更新 — GUIなし直接POST (2026-04-26 確定)
- ロード: `POST /service/IEB010.wpp` `seitocd=<生徒コード>&kyoshitsucd=`
- 更新: 通常の `cmd=regist` POST（formmain 全フィールド + 上書き分）
- JS が後から注入する値（`imdatebirth='YYYY/MM/DD'` 等）を正規表現抽出して補完する必要あり
- 送信前正規化: `postalcd = impostalcd.replace('-','')`, `nyujukudt = imnyujukudt.replace('/','')`, `datebirth = imdatebirth.replace('/','')`
- MCPツール: `sks_internal_update_memo` / `sks_internal_update_fields` / `sks_internal_get_fields`

### 外部生 → 内部生 取り込み (2026-04-27 確定)

GUI フロー: `IEB010` 上部の「外部生 [code] [取込] [検索]」セクションから外部生コードを指定 → 「追加／修正」押下時に外部生→内部生として取り込まれる。途中で「証憑データ確認」(YSPC) ダイアログが出るので OK で確定。

ロードリクエスト（外部生情報を IEB010 に転記）:
```
POST /service/IEB010.wpp
mode=gaibu&kyoshitsucd=<教室コード>&seitocd=<外部生コード>
```
レスポンスは `formmain` が外部生情報（氏名・カナ・住所・電話・保護者氏名・備考・郵便番号 等）で埋まった IEB010 ページ。

登録リクエスト（取り込み確定）:
通常の生徒登録 POST に **`gaibuseicd=<外部生コード>`** を含めるだけ。
サーバ側で内部生コード(`seitocd`)が新規採番され、外部生レコードからの取り込みが完了する。

YSPC ダイアログ相当の値（プログラマブル運用時は固定）:
- `premshubetsu=4` (未登録)
- `premreason=2` (証憑未取得) または `3` (デジタルデバイスなし)
...
筆頭保護者: `rgfamily=1` (保護者1) / `2` (保護者2) ...

キャンペーン種別 (`campaign`) は数値コード。マスタによって変わるので、ロード後の `<select name="campaign">` から動的に表示名→コードを抽出する。

MCPツール:
- `sks_gaibu_preview(gaibuseicd, kyoshitsucd=None)` — ロード結果プレビュー（POSTせず読み取りだけ）
- `sks_convert_gaibu_to_internal(gaibuseicd, nyujukudt, datebirth, parent1_zokugara="父", parent1_is_primary=True, campaign="", campaign_reason="", premshubetsu="4", premreason="2", additional_fields=None, kyoshitsucd=None, dry_run=False)` — 取り込み実行

実例: 2026-04-27 涌井 瑠愛（26G033、講習会生・中2）→ 内部生 `260006` として登録。GUI で初回成功 → MCPツール化。

## 受講履歴登録 (`/service/IEB020.wpp`)

> ⚠ **コース変更/進級済み生の落とし穴**: 学年が進級した既存生のコース変更では、入会時通番(tsuban)のコース表＝**入会時学年**しか出ない。コース変更日の履歴行を**先に登録して新通番を発番**させ、その通番で IF2SHOW を引くと適用日学年のコース表が出る。これを飛ばして読み取りAPIを直叩きすると詰まる（`sks_jugyo_register` の「PS2標準コース自動特定できません」の真因）。迷走の記録と正しいフロー: [sks-failures.md](sks-failures.md) 失敗1。
...
| 表示列 | フォーム要素 name | 型 | 備考 |
|---|---|---|---|
| 削除 | `if2DEL_n` | checkbox | |
| コース名 | `if2CS_n` | select | option text は `{コードcd}：{コース名}` 形式 |
| 回数 | `if2KAISU_n` | text | 週あたり回数 |
| 国 | `if2SELKOKUGO_n` | checkbox | 国語 |
| 数 | `if2SELSUGAKU_n` | checkbox | 数学・算数 |
| 英 | `if2SELEIGO_n` | checkbox | 英語 |
| 理 | `if2SELRIKA_n` | checkbox | 理科 |
| 社 | `if2SELSHAKAI_n` | checkbox | 社会 |
| 金額 | `if2KINGAKU_n` | text | コース＋回数から自動算出 |
| (単価) | `if2TANKA_n` | text | 単価ベース |
| (割引) | `if2WARI_n` | text | 割引 |

### 複合教科コースの扱い ⭐

複数教科をまとめて扱う「N教科コース」が存在する。教科ごとに別行を立てない。

- ❌ 1行=理科、2行=社会 のように**教科ごとに分けない**
- ✅ コース select で `〇〇・2教科` のような複合コースを1つ選び、その行の `if2SELRIKA_n` / `if2SELSHAKAI_n` などを**両方ONにする**

option text の末尾に `・2教科` `・3教科` などの教科数表記があるコースが複合型。教科ごとに個別コースが用意されているケースもあり、コース選択肢の命名規則を最初に確認すること。

### form 構造の落とし穴

`fmif2` form の直接の elements には hidden (`mode=if2`, `cmd`, `seitocd`, `tsuban`, `nyujukudt`) と `cmd_if2add` / 元に戻す ボタンしか含まれない。各コース行の入力欄 (`if2CS_n` 等) は **form タグの外側**に配置されている。

→ 要素取得は `fmif2.querySelector(...)` ではなく、iframe document 全体に対する `contentDocument.querySelectorAll('[name^="if2"]')` を使う。

### 料金テーブル（HTML埋め込みhidden）

選択したコースcd `C` に対応して以下のhidden inputsが料金算出に使われる:

| name | 内容 |
|---|---|
| `PSS{C}` | 選択フラグ |
| `MAXKOMA{C}` | 最大コマ数 |
| `TANKA{C}` | 単価（週1回ぶんの基準額） |
| `WGK{N}_{C}` (N=1..18) | 週N回受講時の**割引額**（複数受講の値引き、累積） |

**金額の算出式（実測）:** `金額 = TANKA{C} × 回数 − WGK{回数}_{C}`
例) コース12100(PS2･中3): TANKA=17910, WGK1=0, WGK2=2860
- 回数1 → 17910×1 − 0 = **17,910**
- 回数2 → 17910×2 − 2860 = **32,960**

（`WGK{N}` をそのまま金額にコピーするのではない点に注意。WGK は割引額。）

### アクションボタン
| ボタン | name | 説明 |
|---|---|---|
| 追加 (履歴行) | `cmd_if1add` | 受講履歴をローカル状態に追加 |
| 削除 (履歴行) | `cmd_del` | 選択された履歴行を削除 |
| 追加／修正 (コース) | `cmd_if2add` | コース情報行をローカル状態に確定 |
| 元に戻す | (name無し) | コース行の変更取り消し |
| 終了 | | モーダルを閉じる |

### 操作手順（GUI / CDP共通）

1. IEB010 で生徒表示 → 「授業登録」ボタンで IEB020 モーダル展開
2. fmif1 で `imhisdt`/`jukorireki`/`biko` 入力 → `cmd_if1add` クリック → 履歴行追加
3. `jukorireki` が `02/05/06` の場合は **fmif2 コース情報の入力が必須**。未入力で送ると `E10044 コース情報が登録されていません` で拒否
4. fmif2 のコース行で `if2CS_n` を選択 → 該当する教科チェックを ON → `if2KAISU_n` 入力 → `cmd_if2add` でコース行確定

**実測:** `cmd_if1add` / `cmd_if2add` はローカル状態だけでなく、その場で `cmd=ax` の AJAX を発行して**サーバーに永続化**する（別途の最終登録ボタンは不要。名簿の 科目①/金額① に即反映される）。

### AJAX パラメータ書式（GUIなし実装）

各ボタンが叩く実エンドポイント（`GET /service/IEB020.wpp?cmd=ax&param=...`、`|` `:` はURLエンコード）:

| 操作 | param 書式 |
|---|---|
| 受講履歴を追加 (`cmd_if1add`) | `IF1regist\|{生徒cd}\|\|{入会日YYYYMMDD}\|{日付YYYYMMDD}\|{受講履歴cd}\|` |
| コース表を表示 (読取) | `IF2SHOW\|{生徒cd}\|01\|{入会日YYYYMMDD}` |
| コースを確定 (`cmd_if2add`) | `IF2ADD\|{生徒cd}\|01\|{入会日YYYYMMDD}\|{行}` |

- `{入会日}` は IEB010 の `nyujukudt`（契約日）。受講履歴の `{日付}` は授業開始日。
- 行 = `{idx}:{コースcd}:{回数}:{国}:{数}:{英}:{理}:{社}:{金額}`（国数英理社は 0/1、金額は上式で算出）
  - 例) `0:12100:1:0:1:0:0:0:17910`（PS2･中3 / 週1 / 数のみ / ¥17,910）
- `param` 中の `01` は対象の受講履歴行番号。新規生徒の最初の1件なら `01`。
- `IF2SHOW` は常に空の入力フォームを返す（保存済みコースは出ない）。二重登録防止には名簿の `科目①` を事前確認する。

### MCP プログラマブル等価フロー

`servers/sks/server.py` の以下ツールで GUIなしに登録できる:

- `sks_jugyo_show(seitocd)` — 現在の受講(コース①②③)＋選択可能コース一覧(コード:名称・単価)を返す（読取専用）
- `sks_jugyo_register(seitocd, subjects, kaisu=None, course_code="", start_date="", rireki="02", dry_run=True, allow_existing=False)`
  - `subjects` は国数英理社の文字列（例 `"数"`, `"数英"`）。`kaisu` 省略時は科目数
  - `course_code` 省略時は学年から PS2 標準コースを自動選択
  - 金額は `単価×回数−割引` で自動算出
  - **既定 `dry_run=True`**（月謝発生の不可逆操作のため、まず内容だけ返す）。実登録は `dry_run=False`
  - 既に `科目①` がある生徒は拒否（`allow_existing=True` で続行）
  - 内部で `IF1regist`→`IF2ADD` を順に発行し、名簿の `科目①/金額①` で結果検証する
...
| 履歴 | `showhist()` | `window.open('PcsHistory.do?p={教室コード}_250022_'+KK)` |
...
- SumatraPDF: `C:\Users\{user}\AppData\Local\SumatraPDF\SumatraPDF.exe`
...
| `kyoshitsucd` | 教室コード | text(readonly) | 固定: `{教室コード}` |
| `kyoshitsusm` | 教室名 | text(readonly) | 固定: `{教室名}` |
...
| `ad1` | 住所1（都道府県市区町村） | text | 例: `埼玉県{市区名}` |
| `ad2` | 住所2（番地） | text | 例: `○○町1-2-3` |
| `ad3` | 住所3（建物名等） | text | 例: `{建物名}{教室名}210` |
| `telno` | 電話番号 | tel | |
| `schoolsm` | 学校名 | text | |
...
#### 行選択
行の `onclick="sel(this,'{教室コード}:{No}')"` で選択。codeは `{教室コード}:{No}` 形式。
...
3. `tr[onclick="sel(this,'{教室コード}:XXXX')"]` をclick → 行選択
...
| フィールド | 型 | 備考 |
|---|---|---|
| 教室コード | readonly | `{教室コード}` |
| 教室名 | readonly | `{教室名}` |
| ログインID | テキスト | 8桁（教室コード{教室コード} + 4桁）。検索/表示ボタンあり |
| 氏名 | テキスト | |
| フリガナ | テキスト | |
...
#### {教室名} 慣例（運用ルール）
- **支払期日 (`shiharaidt`) = 振込票発行日 + 20日**
  - 例: 振込票発行日 2026/04/27 → 支払期日 2026/05/17
  - これは英検検定料を含む単発請求での標準運用
...
## コンビニ未入金一覧 (`/service/IEV140.wpp`) / 入金済一覧 (`/service/IEV250.wpp`)

振込票（コンビニ払込）を発行済みで**まだ入金されていない**請求の一覧。
MCPツール `sks_cvs_unpaid_list` / `sks_cvs_paid_list` で自動化済み。

### データ取得は xmlloads (Ajax)

画面の初期表示は `forinit()` → `doreload()` → `xmlloads(0,"IEV140if","VIEW")`。
**`"IEV140if"` は差し込み先の div id であって URL ではない。**
`IEV140if.wpp` のような URL は存在せず、叩くと 404 になる。

```
GET /service/IEV140.wpp?cmd=ax&param=VIEW
GET /service/IEV250.wpp?cmd=ax&param=VIEW
```

- 事前に `GET /service/IEV140.wpp` でセッション確立が必要
- **POST ではなく GET**。POST だと空の外枠 HTML が返るだけで表が付いてこない
- レスポンスは `"<TABLE>…|<script>…"` の形。`|` の**前半**が表本体

### ⚠ タグが大文字

返ってくる断片は `<TR>` `<TD>` と**すべて大文字**。
`re.findall(r"<tr .*?</tr>", body)` のように小文字だけで書くと **0 件**になり、
「データが無い」と誤読する。`re.I` を必ず付ける。
...
| 3 | 区分 | 内部生 / 外部生 |
| 4 | 学年 | |
| 5 | 生徒 | `{生徒コード}:{氏名}` の形。`:` で分割する |
| 6 | 請求区分 | 振込 / ワイドネット |
| 7 | 調整区分 | 調整のときだけ入る |
| 8 | 振込票発行日 | |
| 9 | 請求金額合計 | CVS手数料 330 円込み |
| 10 | 確定日 | CVS 確定した年月日 |
| 11 | 送付先 | |

支払期限前の行は `<TR class=' yellow'>`、**期限超過は `class=' red'`**。
ツール側は日付比較で `overdue` を立てているので class に依存しない。

入金が一覧に反映されるのは**翌営業日以降**。直近に支払われた分はまだ残って見える。
...
### 備考
{教室名}ではBCSは実質未使用。機能としてはDVD注文・視聴履歴追加が可能。
