...
- GET `/console/editions/{教室ID}/edit/{版ID}` — 教室ページ編集（345フィールド）
  - ボタン: プレビュー / コピーして新規作成 / 版管理に戻る / 一時保存 / 保存後承認依頼 / 削除 / 公開中ページとの差分表示
  - タブ切替: アンカーリンク（#detail, #info, #staff, #experience, #course）。ページリロードなし、同一フォーム内。POSTは全タブ分を一括送信
...
- キャンペーンバナー (select) × 5枠 — 選択肢: `--`, `無料体験授業 受付中`, `学力診断テスト 受付中`, `入会金無料 実施中`, `90分×4回￥3,300（税込）`
- キャンペーン条件 (text) × 5枠 — バナーごとの適用条件
- 特別なお知らせ — 見出し文(textarea) + 本文(textarea, **HTML可**) + 画像 + 説明文(textarea) + 表示チェックボックス
...
- info_details[free][N][header] (textarea, **HTML可**) — フリー枠見出し（**3枠あり**、N=0,1,2）
- info_details[free][N][content] (textarea) — フリー枠内容
- 各セクションに表示順位 (select, 0-10) — 0=非表示、1-10で表示順を制御。合計6セクション（campaign, topics, classroom, free×3）の順序を自由に入替可能
- 固定情報 (radio) — 表示/非表示
...
- 各体験談: 表示順位(select, 0-10) + 氏名(text) + 学年(select) + 画像(file) + イラスト代用(select) + 見出し文(textarea) + 本文(textarea, **HTML可**)
- イラスト代用 (select) — `なし` + 21種類（小学生男子1-3、小学生女子1-3、中学生男子1-4、中学生女子1-4、高校生男子1-3、高校生女子1-3）
- 最大10名分のスロット

### コース・時間割・料金タブ (classroom_course)
- course_info (textarea) — コース概要テキスト
- original_course_content (textarea) — 独自コース内容
- timetable_flag (radio) — 時間割 表示/非表示
- timetable_info (textarea, **HTML可**) — 時間割紹介文
- timetable1 (textarea, **CKEditor**) — 時間割HTMLテーブル
- timetable_annotation (textarea) — 時間割注釈
- fee_flag (radio) — 授業料 表示/非表示
- fee_info (textarea, **HTML可**) — 授業料紹介文
- fee1 (textarea, **CKEditor**) — 授業料HTML
- fee_annotation (textarea) — 授業料注釈
...
- 公式フォームURL: `https://www.schoolie-net.jp/form/entry.php?mode=2&ccd={教室コード}`
