...
系統図画面は別ドメイン `schoolie-tacs-ssk2.mirrei.dev` で動くJava Servlet（.do）。
...
- URL: `https://schoolie-tacs-ssk2.mirrei.dev/pcs/PcsMenu.do`
- 同タブ遷移（pcs.wppから直接）
...
|---|---|---|
| (empty) | 未選択 | 初期状態 |
| `clYellow` | **問題作成直後（黄）** | ①問題作成で付与 |
| `clNavy` | 問題作成済・前回テスト分（紺） | 次のテストサイクルに進むと黄→紺に変化 |
| `clLNavy` | 薄紺（前々回以前の問題作成済?） | |
| `clBlue` | 予定登録済（青） | ⑤予定登録で付与 |
| `clRed` | 採点済（赤） | ③採点で付与 |
| `clGreen` | カリキュラム作成済（緑） | ⑥カリキュラム作成で付与 |
| `clGray` | グレー（カリキュラム作成後の完了?） | |
...
→ ⑤予定登録 → [clBlue] ※次回テスト用の単元を青で予約
  SMSG: 「カリキュラム作成を押して下さい。」
  有効ボタン: ⑥

→ ⑥カリキュラム作成 → [clGreen推定]
  月回数(tukikaisu)を入力して登録
...
- URL: `https://schoolie-tacs-ssk2.mirrei.dev/pcs/Omt.do?flag=0&ktzFlag={0|1}`
...
- 単元コードは `{大分類}{小分類}_{連番}` 構造
- stage1(小学): 1201〜1609系、stage2(中学): 1701〜2006系+8701〜9906系、stage3(高校): 1001〜9006系
- 算数・数学で全331単元（stage1:109 + stage2:101 + stage3:121）
...
- URL: `https://schoolie-tacs-ssk2.mirrei.dev/pcs/PcsSaiten.do`
- フォームフィールド:
...
- URL: `https://schoolie-tacs-ssk2.mirrei.dev/pcs/PcsCurriculum.do`
...
2. レスポンスHTML内の `fmpost2` フォーム（action=`https://schoolie-tacs-ssk2.mirrei.dev/pcs/Pcs.do`）の全hiddenフィールドをPOST
...
`doCheckboxes(GMODE, STAGES[i], "checked")` が全チェックボックスの状態を改行区切りで返す:
```
{単元コード}|{0or1}|{色}||
```
- `0` = 未チェック、`1` = チェック済み
...
- **未解決**: PythonからPOSTするとkaisu/SMSGが空で返る。セッション確立→採点は成功するが、予定登録→問題作成のPOSTが効いていない。原因調査中。checksの形式は正しいが、fmpostでのreload（更新）やform1のsubmitのタイミング・順序に問題がある可能性

### PCS 1回分の手順
1. PCS画面で生徒番号入力 → 検索 → 教科選択 → 系統図
2. ⑤予定登録（単元チェック後）— **window.open上書きはしない**
3. ⑥カリキュラム作成 → 別タブで月4回入力 → 登録 → タブ閉じる — **直前にwindow.open上書き**
4. 更新ボタン → 次のテストに進む
5. 単元チェックボックスを選択（手動判断）
6. ①問題作成 — **直前にwindow.open上書き**（ポップアップの場合）
7. ②問題印刷 → PDF取得 → SumatraPDFでA3カラー片面印刷
8. 解答印刷 → PDF取得 → ページ数に応じてNin1でA3印刷
9. ③採点 → 別タブで正解数入力 → 登録/修正 → タブ閉じる — **直前にwindow.open上書き**

### GUIなしPCS手順（Python/MCP）
1. SKSログイン（AES暗号化3ステップ）
2. `pcs.wpp` → `pcs.wpp?cmd=ax&param={生徒番号}` で生徒情報取得
3. `pcs_start.wpp` POST → `Pcs.do` POST（2段階）でssk2セッション確立
4. `PcsPrintMondai.do?cmd=print&opt1=1&bgFlag=1` GET → 問題PDF
5. `PcsPrintMondai.do?cmd=print&opt1=2&bgFlag=1` GET → 解答PDF
6. SumatraPDFでローカル印刷
7. `PcsSaiten.do` POST → 採点登録
8. `PcsCurriculum.do` POST → カリキュラム登録
...
### 概要
nssk.wppにアクセスすると**別ドメイン `schoolie-tacs-ssk1.mirrei.dev`** にリダイレクトされる。
...
| WebSKSメニュー | メインメニューに戻る | `https://schoolie-tacs.mirrei.dev/service/` |

※ ベースURL: `https://schoolie-tacs-ssk1.mirrei.dev/nssk/`
