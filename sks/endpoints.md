...
```
- `0` = 未チェック、`1` = チェック済み
- 色: 現在の状態を表す色クラス（clYellow=選択済, clGreen=○, clBlue=△, clNavy=×, clRed=予定登録済, clGray=完了, 空=未選択）
...
- **checksのcname（重要）**: サーバーは色クラスで操作を判別する
  ...
  - 改行コードは `\n`（LF）でも `\r\n`（CRLF）でも動作する（ブラウザはCRLFで送信）
  - **初回PCS（一度もサイクルを完了していない生徒）はPython POSTで問題作成が効かない場合がある**。原因未特定。回避策: Chrome DevToolsで初回だけ実行するか、2回POST（1回目で状態が変わり2回目で通る）
