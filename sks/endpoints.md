...
- **checksのcname（重要）**: サーバーは色クラスで操作を判別する
  ...
  - 予定登録(mode=updy): 選択単元に **`clBlue`** が必須。`{key}|1|clBlue||`
  - 非選択単元は `{key}|0|||` でOK（cnameは空）
  - 既に色がついている単元（clNavy, clGray等）はその色をそのまま送る。tag/ctagも保持（例: `{key}|1|clNavy|紺|`）
  - ブラウザでは `tgSelectChange()` が自動的にcnameをclYellowに設定するが、PythonからPOSTする場合は明示的に設定が必要
