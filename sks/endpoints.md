...
- **⑤予定登録のdisabled**: UIではdisabledでも、採点後にカリキュラム作成が直接可能な場合がある（予定登録をスキップ可能）。サーバー側のバリデーションはUI側より緩い。本当に押せないボタンと、disabled属性を外せば押せるボタンがある（要調査）
- **checksのcname**: 予定登録(mode=updy)ではclBlueが必須（Chrome経由ではtgSelectChangeで設定）。問題作成(mode=updm)ではcnameは空でOK
